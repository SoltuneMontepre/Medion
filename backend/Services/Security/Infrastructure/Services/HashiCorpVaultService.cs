...
public class HashiCorpVaultService : IVaultService
{
    ...
    {
        _logger = logger;
        // Support both "Vault:Address" and "Vault:Url" for flexibility
        _vaultAddress = configuration["Vault:Address"]
                        ?? configuration["Vault:Url"]
                        ?? throw new InvalidOperationException("Vault:Address or Vault:Url not configured");
        // Support both "Vault:Token" and "Vault:Auth:Token" for flexibility
        _vaultToken = configuration["Vault:Token"]
                      ?? configuration["Vault:Auth:Token"]
                      ?? throw new InvalidOperationException("Vault:Token or Vault:Auth:Token not configured");
    }
